    ...
    public class ApplyController : BaseController
    {
        ...
        public JsonResult CheckHasApplyPay()
        {
            ...
            try
            {
                ...
                //判断用户是否完成缴费
                int applaycount = ApplyOrderBLL.Instance.GetList(new ApplyOrderEntity() { MemberId = LoginUser.UserId, Status = (int)RCHL.Model.Enums.ApplyStatus.已支付 }).Count();
                if (applaycount == 0)
                {
                    result.Code = -2;
        ...
        public JsonResult CreateOrder(string data)
        {
            ...
            try
            {
                lock (objLock)
                {
                    ...
                    //送积分
                    GivePointBLL.GivePoint(RCHL.Model.Enums.OperationType.预约驾校成功, LoginUser.UserId, double.Parse(order.TotalMoney.ToString()), order.ApplyOrderNo);
                    ...
                    FinaceBLL.Instance.Add(new FinaceEntity()
                    {
                        ...
                        PayType = (int)RCHL.Model.Enums.PayType.微信支付,
                    ...
                    //member.LevId = ((int)RCHL.Model.Enums.UserType.VIP会员).ToString();
                    //member.LevName = RCHL.Model.Enums.UserType.VIP会员.ToString();
